...
def solve_knights_tour(x, y, move_i, board, N, x_move, y_move):
    """Recursively solve the Knight's Tour problem.
    ...
    for i in range(8):
        ...
        if is_valid_move(next_x, next_y, board, N):
            ...
            if solve_knights_tour(next_x, next_y, move_i + 1, board, N, x_move, y_move):
                return True
            # Backtracking
...
def knights_tour(N, start_x=0, start_y=0):
    ...
    """
    print("Starting Knight's Tour on a {}x{} board from position ({}, {})".format(N, N, start_x, start_y))
    ...
    if not solve_knights_tour(start_x, start_y, 1, board, N, x_move, y_move):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python knightstour.py <board_size>")
        sys.exit(1)
